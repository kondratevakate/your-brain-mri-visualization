"""Reproduce the pink furry dog-shake (random two-layer fur, inertial whip).

    python contributions/3d-art/kondratevakate-furry-dogshake/build.py

Hyphenated folder -> run as a FILE; bootstrap puts repo root on sys.path.
Thin wrapper around rendering/examples/16_furry_dogshake.py.
"""
import sys
...
PINK = (0.96, 0.58, 0.66, 1.0)
DARK = (0.45, 0.20, 0.28, 1.0)
...
def build():
    ...
    u = sample_seeds(shell, 8000, seed=2)
    t = sample_seeds(shell, 9000, seed=9)
    layers = [
        dict(name="Undercoat", seeds=u, whip_scale=0.35,
             pin_kwargs=dict(length=7.0, base_radius=0.45, tip_radius=0.14,
                             segments=4, sides=4)),
        dict(name="Topcoat", seeds=t, whip_scale=1.25,
    ...
    def scene_factory(shell_r, meshes):
        return Scene(layers=[
            Layer(shell_r, Material(kind="stone", color=DARK, roughness=0.75), name="Skin"),
            Layer(meshes["Undercoat"], Material(kind="stone", color=DARK, roughness=0.7), name="Undercoat"),
            Layer(meshes["Topcoat"], Material(kind="stone", color=PINK, roughness=0.5), name="Topcoat"),
    ...
    sched = shake_schedule(48, shake_deg=26.0, n_shakes=3, whip_deg=95.0,
                           w0=4.5, zeta=0.085)
    render_fur_gif(shell, layers, scene_factory, HERE / "furry_dogshake.gif",
